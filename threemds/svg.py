...
def create_svg_from_vmobject(vmobject: VMobject, file_name: str | Path = None, trim=True):
...
def create_svg_from_vgroup(vgroup: VGroup, file_name: str | Path = None, trim=True):
    if file_name is None:
        file_name = tempfile.mktemp(dir="media", suffix=".svg")
    file_name = Path(file_name).absolute()
